...
namespace ofxModule {
	...
	ModuleLoader::ModuleLoader(ModuleRunner* moduleRunner_) {
		...
		if (jsonFile.exists()) {
			...
			try {
				jsonFile >> json;
				if (json["settingsPath"] != nullptr && json["settingsPath"] != "") {
					configPath = json["settingsPath"].get<string>() + "/modules.json";
				}
			}
			catch (std::exception & e) {
				ofLogError("ofLoadJson") << "Error loading json from settingsPath.json: " << e.what();
			}
		}
		
		config = ofLoadJson(configPath);
	}
			...
			for (auto& input : inputs)
			{
				for (int i = 0; i < input.second.size(); ++i) {
					if (idMap.find(input.second[i]) != idMap.end()) {
						input.first->registerModule(idMap[input.second[i]]);
	...
	void ModuleLoader::createInputList(ModuleRunnable* module) {
		...
		for (auto& jmodule : config) {
			if (jmodule["type"] == module->getModuleName() || (jmodule["id"]!= nullptr && jmodule["id"] == module->getModuleName())) {
